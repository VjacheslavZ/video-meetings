...
- `src/meeting-file/` — file attachments on a meeting; not its own `@Module`, registered directly into `MeetingModule`'s `controllers`/`providers` (per-domain module split wasn't warranted for a controller/service/guard trio this small, and it needs `MeetingService` from the same module for access checks).
  - `meeting-file.controller.ts` — `MeetingFileController`, `@Controller('meetings/:meetingId/files')`, guarded by `JwtAuthGuard` + `MeetingFileAccessGuard`. `POST /` (`FilesInterceptor('files', ...)`, multi-file, `multerOptions` from `upload/multer.config.ts`), `GET /` (list), `GET /:fileId` (streams the file back as a `StreamableFile`, `Content-Type` from the stored `mimeType`, `Content-Disposition` built by `upload/content-disposition.util.ts`'s `buildContentDisposition` — RFC5987-encodes the untrusted original `filename`), `DELETE /:fileId` (deletes the file, 200 with no body).
  - `meeting-file.service.ts` — `MeetingFileService`. `saveUploaded` writes one `MeetingFile` row per uploaded file (`storedName` = multer's generated on-disk filename, `filename` = original client name) inside a `$transaction`; throws `BadRequestException` if the files array is empty. `findAllForMeeting` lists a meeting's files ordered by `createdAt`. `getOneForMeeting` looks up a file scoped to its `meetingId` (404 if the file doesn't exist or belongs to a different meeting). `deleteFile` looks up the file scoped to its `meetingId` (404 if it doesn't exist there), throws `ForbiddenException` unless the caller is the file's uploader or the meeting's owner (fetched via an `include: { meeting: true }` on the same lookup), then deletes the `MeetingFile` row and unlinks the on-disk file (`UPLOADS_DIR`/`storedName`). All three read/list/upload responses include `uploaderEmail` (joined from `uploadedBy`), mirroring how `MeetingParticipant` responses already expose `email` instead of a bare `userId`.
...
Phase 1 (data model, upload, list, download) and Phase 2 (deletion — see `src/meeting-file/` above) are implemented. The web UI (Phase 3) is still pending — use this research for its implementation: @docs/research-meeting-file-upload-technical-approach.md
