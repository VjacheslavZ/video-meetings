...
- `src/app/(app)/meetings/[id]/page.tsx` — meeting detail page: date, role/status badge; Accept/Decline buttons for a pending invitee; a participant list with per-row status; owner-only controls to invite more participants (inline form) and remove one (icon button behind a HeroUI `AlertDialog` confirmation, since removal is immediate and access-affecting). A user with no access gets a 404-style message rather than raw API errors. Also a "Files" section: a list of meeting files (filename, uploader, size, upload date) with per-file download and delete (owner or uploader only, behind the same `AlertDialog` confirmation pattern) icon buttons, and a multi-file upload control (hidden native `<input type="file" multiple>` triggered via a button) with busy/error state.
...
- `src/lib/api.ts` — `fetch`-based client for `apps/api` (register/login/get meetings/create meeting/get meeting by id/accept/decline/add participants/remove participant/upload meeting files/list meeting files/delete meeting file/download meeting file), throwing `ApiError` (with `status`) on non-2xx responses. `uploadMeetingFiles` sends `multipart/form-data` via `FormData`; `downloadMeetingFile` fetches the file as a blob and saves it via a temporary object URL, since the endpoint requires an `Authorization` header a plain `<a href>` can't send.
